...
| `go2.py` | `Go2MujocoHAL` + `GO2_DESCRIPTION` — Unitree Go2 quadruped (12-DoF) MuJoCo digital twin on the `mujoco_menagerie/unitree_go2` MJCF (BSD-3-Clause / Unitree). Sim-only: H1-style software PD over torque motors; idle ticks PD-hold `_hold_targets` (base `idle_step` would fold the calves). Gravity pinned off in `hal.parameters.defaults`; `scenes/deploy/go2_walk.yaml` overlays it on for the rsl-rl ONNX skill. Publishes `base_pose_6dof` + `base_twist` from the free joint. Spawn snaps to Hub `default_joint_pos` (hip ±0.1). RGB: `front` + viz-only `top`. No `BODY_TWIST`, no real HAL. |
| `go2_z1.py` | `Go2Z1MujocoHAL` — 19-DoF composite (12 Go2 legs + 6 Z1 arm + jaw). Mixed actuators: torque-PD legs, position-servo arm. Joint order is load-bearing (legs 0–11 byte-identical to `robots/go2`) so the 12-DoF rsl-rl skill hold-pads onto the arm. Sticky `_arm_hold_pose` (Recalibrate / `arm_ready`) is qpos-snapped on locomotion + idle. MJCF from `scene_defaults.composition` → `compose_mounted_arm_mjcf`. Sim-only (`hal.real` null). |
...
| Unitree Go2 (MuJoCo digital twin) | `Go2MujocoHAL` | ✓ sim (`tests/sim/test_go2_hal_mujoco.py`) | 12-DoF quadruped on the `mujoco_menagerie/unitree_go2` MJCF. Software PD, Hub stand (hip ±0.1), free-joint pose/twist for rsl-rl, spliced `front` + viz-only `top` RGB. Default gravity off (`go2_bench`); `go2_walk` turns it on for the rsl-rl ONNX skill. No `BODY_TWIST` / real HAL / Go2 Edu. |
| Unitree Go2 + Z1 (MuJoCo digital twin) | `Go2Z1MujocoHAL` | ✓ sim (`tests/sim/test_go2_z1_hal_mujoco.py`) | 19-DoF composite (legs + Z1 arm + jaw). Mixed torque-PD / position-servo. Same 12-DoF rsl-rl skill via hold-pad; Recalibrate / `arm_ready` sticky hold is qpos-snapped during walk. `scenes/deploy/go2_z1_walk.yaml`. Sim-only. |
