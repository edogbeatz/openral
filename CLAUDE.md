...
> - Operator `/simple` app → [`docs/quickstart/dashboard.md`](docs/quickstart/dashboard.md). **When asked to start the app / dashboard / cricket / `/simple`, run it this turn** — do not print the command and wait. Reuse `http://127.0.0.1:4318/healthz` if live; else `just dashboard` from repo root (write-controls). Then `POST /api/demo/cricket/start` if the twin is down (already-running is success). Canonical: [`.agents/skills/go2-foxglove-view/scripts/start-app.sh`](.agents/skills/go2-foxglove-view/scripts/start-app.sh). Skill [`.agents/skills/go2-foxglove-view/SKILL.md`](.agents/skills/go2-foxglove-view/SKILL.md). Do not steal tunnels; do not `pkill` the graph.
...
**ROS 2:** Lifecycle nodes for anything stateful. QoS by data class — sensors `BEST_EFFORT, VOLATILE, KEEP_LAST=5–10`; control `RELIABLE, VOLATILE, KEEP_LAST=1`; description/static `RELIABLE, TRANSIENT_LOCAL, KEEP_LAST=1`; safety/e-stop `RELIABLE, VOLATILE, KEEP_LAST=10`; action feedback `BEST_EFFORT`. Actions for >100 ms or cancellable work; services for instantaneous queries; topics for streams. Composable nodes for intra-process zero-copy. TF2 is the only source of coordinate frames — never inline a 4×4 matrix, never hardcode a `frame_id`. Middleware: Fast DDS is the host/CLI default (ROS 2 Jazzy). Container images set `RMW_IMPLEMENTATION=rmw_cyclonedds_cpp`. Zenoh is opt-in via `RMW_IMPLEMENTATION=rmw_zenoh_cpp`. Host deploys stay on Fast DDS because Jazzy's `lifecycle_event_manager` has a Cyclone deserialisation race on the auto-CONFIGURE → ACTIVATE chain that takes down `prompt_router` — see `_apply_rmw_default`.
...
*End of CLAUDE.md. Last reviewed: 2026-09-19 (`/simple` chat is sibling Acquire probe/ask — first `allow_adapt: false`, ask before remap, no `/openral/prompt` publish, SKILL empty until fit/adapted propose, UNIT switch chats `unit changed.` only when the occupant actually changes, a base tip past 1.0 rad chats `the unit fell. this skill needs retraining or finetuning.` once until RESET, Load auto-stands so Apply is ready, footer RESET is Recalibrate, `#chat-apply` APPLYING… when stood (STANDING… only if not; same button is STOP / STOPPING… while running); hop on Go2+Z1 is payload reject; **when asked to start the app, run `.agents/skills/go2-foxglove-view/scripts/start-app.sh` this turn** (`just dashboard` then cricket Start; reuse `:4318/healthz`; do not print and wait); laptop `openral dashboard` loads `~/.openral/dashboard.env` into empty `ACQUIRE_API_*` (`just dashboard-acquire-env` seeds it; no library URL default); Go2 `front` is `sim_render: false` — dashboards stream `top` only; `/simple` paints MJPEG with fetch + canvas keep-latest; laptop seeds from cricket `/api/state` thumbs then splices `:14318` stream; `GET /api/config` `robot_id` overlays the tunneled twin; empty UNIT is **Load the unit**; live chrome is full-width OP_CAL + conn; Reset is one POST `/api/demo/recalibrate`; Stop cancel-all is idempotent; live `graph_running` clears a stale sshd-race `start_error`; `GET /` 307s to `/simple` (classic root UI is not started); cricket attach/reload sets `OPENRAL_CRICKET_IDLE_S=0`). Stale = file an issue.*
