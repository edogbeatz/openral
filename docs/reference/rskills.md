...
| [`rsl-rl-onnx-go2-velocity-flat`](https://github.com/OpenRAL/openral/tree/master/rskills/rsl-rl-onnx-go2-velocity-flat/) | Isaac Lab / Unitree rsl-rl ONNX Go2 locomotion (`model_family: rsl_rl_onnx`) | `go2` (also `go2_z1` via 12→19 hold-pad) | BSD-3-Clause weights — proprio-only 12-D `JOINT_POSITION`; not SmolVLA; `go2_walk` / `go2_z1_walk` |
