...
|---|---|---|---|---|---|---|---|---|
| `diasAiMaster/unitree-go2-velocity-flat` | MuJoCo (`Go2MujocoHAL` / `Go2Z1MujocoHAL`) | `go2` | rsl-rl obs (ang vel + projected gravity + cmd + joint_pos_rel + joint_vel_rel + last_action) | none (proprio only) | **12-D** `JOINT_POSITION` | `rskills/rsl-rl-onnx-go2-velocity-flat` | BSD-3-Clause | `scenes/deploy/go2_walk.yaml` (gravity on). Same skill on `go2_z1` via runner + HAL hold-pad 12→19 (`go2_z1_walk`). Last 3 s of `horizon_s` command `[0,0,0]` so the dog stands before the goal ends. Does not claim gait quality. ACM on those scenes is stand-justified, not gait-swept. |
| mjlab hop (`Renkunzhao/legged_rl_deploy` `policies/go2/mjlab/hop`) | MuJoCo (`Go2MujocoHAL` / `Go2Z1MujocoHAL`) | `go2` | 470-D term-major history-10 (gait_phase_2 + cmd + ang vel + euler RPY + joints + last action) | none (proprio only) | **12-D** `JOINT_POSITION` | `rskills/rsl-rl-onnx-go2-hop-flat` | unknown (not vendored; `onnx_url` fetch) | Planted crouch on this HAL (`air_ticks=0`). Not the dashboard hop picker. |
| gym spring_jump (`Renkunzhao/legged_rl_deploy` `policies/go2/My_unitree_go2_gym/spring_jump`) | MuJoCo (`Go2MujocoHAL` / `Go2Z1MujocoHAL`) | `go2` | 470-D frame-major history-10 (constants + joystick A + scaled ang vel / euler / joints / last action) | none (proprio only) | **12-D** `JOINT_POSITION` | `rskills/rsl-rl-onnx-go2-spring-jump` | unknown (not vendored; `onnx_url` fetch) | One-shot jump under walk PD (`air_ticks≈15`, max foot ≈0.34 m, `z_max≈0.53 m`). Dashboard **hop** Apply. Hold-pads 12→19 on `go2_z1`. |
| scripted hop (`gait: jump`) | MuJoCo (`Go2MujocoHAL` / `Go2Z1MujocoHAL`) | `go2` | unused (open-loop) | none | **12-D** `JOINT_POSITION` | `rskills/rskill-zero-go2-hop-fp32` | Apache-2.0 | Crouch-extend-tuck-land under walk PD. In-tree fallback, not the picker. Hold-pads 12→19 on `go2_z1`. |
