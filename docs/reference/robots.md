...
| Unitree Go2 quadruped | [`robots/go2/`](https://github.com/OpenRAL/openral/tree/master/robots/go2/) | `Go2MujocoHAL` (sim-only; H1-style software PD; `go2_bench` gravity-off pipe proof, `go2_walk` + rsl-rl ONNX for gait; no real HAL) | ✓ sim |
| Unitree Go2 + Z1 | [`robots/go2_z1/`](https://github.com/OpenRAL/openral/tree/master/robots/go2_z1/) | `Go2Z1MujocoHAL` (19-DoF: 12 torque-PD legs + 6 position-servo arm + jaw; rsl-rl walk hold-pads 12→19 and qpos-snaps the sticky Recalibrate / `arm_ready` hold; `rskill-zero-go2_z1-arm_ready-fp32` commands the arm) | ✓ sim |
